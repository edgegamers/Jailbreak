...
public class LastRequest : AbstractCommand {
  ...
  public LastRequest(IServiceProvider services, BasePlugin plugin) :
    ...
    _menuSelector = new LastRequestMenuSelector(
    ...
    switch (info.ArgCount) {
      case 3 when executor != null: {
        if (executor.Team == CsTeam.Terrorist)
          _manager.InitiateLastRequest(executor, fromPlayer.First(),
            type.Value);
        else // They aren't necessarily on different teams, but this is debug so that's OK
          _manager.InitiateLastRequest(fromPlayer.First(), executor,
            type.Value);
        return;
      }
      case 4: {
        ...
        _manager.InitiateLastRequest(fromPlayer.First(), targetPlayer.First(),
          type.Value);
        break;
      }
    }
