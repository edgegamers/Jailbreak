...
public class LogDamageListeners : IPluginBehavior {
  ...
  public LogDamageListeners(IRichLogService logs) { _logs = logs; }
  ...
  public HookResult OnPlayerHurt(EventPlayerHurt @event, GameEventInfo info) {
    ...
    if (isWorld) {
      if (health > 0)
        _logs.Append("The world hurt", _logs.Player(player),
          $"for {health} damage");
      else
        _logs.Append("The world killed", _logs.Player(player));
    } else {
      if (health > 0)
        _logs.Append(_logs.Player(attacker), "hurt", _logs.Player(player),
          $"for {health} damage");
      else
